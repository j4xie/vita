    ...
    try {
      ...
      if (isActuallySuccessful) {
        if (!isCancel) {
          ...
          // 发送报名事件 - Web端使用CustomEvent
          window.dispatchEvent(new CustomEvent('activityRegistered', { 
            detail: { activityId, userId } 
          }));
        } else {
          // 发送取消报名事件 - Web端使用CustomEvent
          window.dispatchEvent(new CustomEvent('activityCancelled', { 
            detail: { activityId, userId } 
          }));
        }
      }
      ...
      console.error(`❌ [Web-PomeloXAPI] 活动${isCancel ? '取消报名' : '报名'}失败:`, error);
      throw error;
    }
  }
