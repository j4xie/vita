...
async function gitDeploy() {
    try {
        ...
        execSync('git add frontend-web-testenv/dist/', { stdio: 'inherit' });

        // 3. 提交构建文件
        console.log('💾 提交构建文件...');
        const timestamp = new Date().toLocaleString('zh-CN');
        execSync(`git commit -m "chore: 更新测试环境构建文件 ${timestamp}"`, { stdio: 'inherit' });
        ...
        console.log('   cp frontend-web-testenv/dist/* test-h5/');
