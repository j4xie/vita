...
export const ParentEmailRegisterStep2Screen: React.FC = () => {
  ...
  useEffect(() => {
    ...
    return () => {
      ...
      console.log('🔄 [ParentEmailRegisterStep2] Component unmounted, cleaning up...');
    };
  }, []);
    ...
    if (loading) {
      const timer = setTimeout(() => {
        if (isMountedRef.current) {
          console.warn('⚠️ [ParentEmailRegisterStep2] Loading timeout after 30s, auto reset');
          setLoading(false);
        }
      }, 30000); // 30秒后自动重置
  ...
  // 调试：检查当前系统语言
  useEffect(() => {
    console.log('🌍 [ParentEmailRegisterStep2] Language detection:', {
  ...
  const sendVerificationCode = async () => {
    ...
    try {
      ...
      if (response.code === 'OK' && response.bizId) {
        console.log('✅ [ParentEmailRegisterStep2] Verification code sent successfully:', {
  ...
  const handleRegister = async () => {
    ...
    if (verificationCode !== backendVerificationCode) {
      console.log('❌ [ParentEmailRegisterStep2] 验证码不匹配:', {
    ...
    console.log('✅ [ParentEmailRegisterStep2] 验证码验证通过，继续注册流程');

    // 🔧 防抖：2秒内只能点击一次
    const now = Date.now();
    if (now - lastSubmitTime < SUBMIT_DEBOUNCE_MS) {
      console.log('⏱️ [ParentEmailRegisterStep2] Debounce: Ignoring duplicate submit');
      return;
    }
    setLastSubmitTime(now);

    setLoading(true);
    console.log('🚀 [ParentEmailRegisterStep2] Starting email verification registration process...');
    ...
    try {
      ...
      if (lastRequestRef.current === requestKey) {
        console.warn('⚠️ [ParentEmailRegisterStep2] Duplicate request detected, ignoring');
        if (isMountedRef.current) {
          setLoading(false);
        }
        return;
      ...
      console.log('[ParentEmailRegisterStep2] Sending registration data:', registrationData);

      // 调用真实的注册API
      const response = await registerUser(registrationData);

      console.log('[ParentEmailRegisterStep2] Registration response:', response);

      if (response.code === 200) {
        console.log('✅ [ParentEmailRegisterStep2] Registration successful! Starting auto login process...');

        try {
          const registrationUserName = registrationData.userName;
          console.log('🔑 [ParentEmailRegisterStep2] Login attempt parameters:', {
          ...
          console.log('📡 [ParentEmailRegisterStep2] Login API response:', {
          ...
          if (loginResult.code === 200 && loginResult.data) {
            console.log('💾 [ParentEmailRegisterStep2] Starting manual token save to AsyncStorage...');
            ...
            console.log('🔄 [ParentEmailRegisterStep2] Calling UserContext.login...');
            await userLogin(loginResult.data.token);
            console.log('✅ [ParentEmailRegisterStep2] Auto login successful! UserContext updated');
    ...
    <SafeAreaView style={styles.container}>
      ...
      <View style={styles.contentView}>
        ...
        <View style={styles.header}>
          ...
          <Text style={styles.headerTitle}>{t('auth.register.parent.form_title')}</Text>
