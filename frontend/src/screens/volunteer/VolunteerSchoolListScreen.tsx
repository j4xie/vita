...
export const VolunteerSchoolListScreen: React.FC = () => {
  ...
  const handleSchoolPress = useCallback((school: any) => {
    ...
    if (!cardLayout) {
      // 没有布局信息，直接切换
      navigation.navigate('VolunteerSchoolDetail' as never, { school } as never);
      return;
    }
  ...
  const startMorphAnimation = useCallback((school: any, layout: any) => {
    ...
    setTimeout(() => {
      ...
      // 使用自定义动画或无动画切换
      navigation.navigate('VolunteerSchoolDetail' as never, { 
        school,
        // 禁用默认slide动画
...
export default VolunteerSchoolListScreen;