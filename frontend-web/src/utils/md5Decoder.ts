...
export const decodeActivityHash = (hash: string): MD5DecodeResult => {
  ...
  // 策略5: 大写十六进制格式
  console.log('🔤 [MD5Decoder] 尝试大写十六进制ID...');
  for (let i = 1; i <= 255; i++) {
    attempts++;
    const text = i.toString(16).toUpperCase();
    const testHash = CryptoJS.MD5(text).toString();
    if (testHash === hash) {
      const result = {
        success: true,
      ...
      console.log('🎯 [MD5Decoder] 破解成功（十六进制）:', result);
      return result;
    }
  }
  
  // 策略6: 特定学校格式
  console.log('🏛️ [MD5Decoder] 尝试学校特定格式...');
  const schoolPrefixes = ['uci_', 'UCLA_', 'USC_', 'Berkeley_', 'Stanford_'];
  for (const prefix of schoolPrefixes) {
    for (let i = 1; i <= 50; i++) {
      attempts++;
      const text = `${prefix}${i}`;
      const testHash = CryptoJS.MD5(text).toString();
      if (testHash === hash) {
        ...
        console.log('🎯 [MD5Decoder] 破解成功（学校格式）:', result);
        return result;
      }
    }
  }
 ...
 * 快速测试特定哈希
 */
export const testSpecificHash = (hash: string, testValue: string): boolean => {
  const computedHash = CryptoJS.MD5(testValue).toString();
