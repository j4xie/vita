...
export const ActivityListScreen: React.FC = () => {
  ...
  const [initialLoading, setInitialLoading] = useState(true); // 恢复初始加载状态
  ...
  useEffect(() => {
    const registrationListener = DeviceEventEmitter.addListener('activityRegistered', (data: { activityId: string }) => {
      ...
      setActivities(prevActivities => {
        console.log('🔄 [ActivityList] 更新前的活动列表:', prevActivities.length, '个活动');
        
        const updatedActivities = prevActivities.map(activity => {
          if (activity.id === data.activityId) {
            console.log('✅ [ActivityList] 找到并更新活动状态:', {
              id: activity.id,
              title: activity.title,
              oldStatus: activity.status,
              newStatus: 'registered'
            });
            return { ...activity, status: 'registered' as const };
          }
          return activity;
        });
        
        console.log('🔄 [ActivityList] 更新后的活动列表:', updatedActivities.length, '个活动');
    ...
    const signinListener = DeviceEventEmitter.addListener('activitySignedIn', (data: { activityId: string }) => {
      console.log('📋 [ActivityList] 收到活动签到成功事件:', data.activityId);
  ...
  }, [activeFilter, searchText, currentLanguage, user?.id]); // 使用user?.id而不是整个user对象避免引用变化
