<template>
  <div class="app-container">
    ...
    <el-table v-loading="loading" :data="invitationList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="邀请码" align="center" prop="invCode" />
    ...
    <el-dialog :title="title" :visible.sync="open" width="500px" append-to-body>
      <el-form ref="form" :model="form" :rules="rules" label-width="80px">
        ...
        <el-form-item label="邀请码" prop="invCode">
          <el-input v-model="form.invCode" placeholder="请输入邀请码" />
        </el-form-item>
      </el-form>
...
<script>
import { listInvitation, getInvitation, delInvitation, addInvitation, updateInvitation, resetInvitation } from "@/api/system/invitation"
...
export default {
  ...
  methods: {
    ...
    /**重新生成邀请码 */
    handleReset(row){
      this.$confirm('重置后，原来的邀请码将不可用，确认重置？', '温馨提示', {
        ...
        console.log(_data)
        resetInvitation(_data).then(response => {
    ...
    handleDelete(row) {
      ...
      this.$modal.confirm('是否确认删除邀请码编号为"' + ids + '"的数据项？').then(function() {
