<template>
  <div class="app-container">
    <el-form :model="queryParams" ref="queryForm" size="small" :inline="true" v-show="showSearch" label-width="68px">
      ...
      <el-form-item label="适用类型" prop="type">
        <el-select v-model="queryParams.type" placeholder="请选择适用类型" clearable style="width: 240px">
          <el-option v-for="t in processTypeList" :key="t.id" :label="t.name" :value="t.id" />
        </el-select>
      </el-form-item>
    ...
    <el-table v-loading="loading" :data="manageList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="流程名称" align="center" v-if="columns.name.visible" :show-overflow-tooltip="true">
        <template slot-scope="scope">
          <span>{{ scope.row.progressName || scope.row.name }}</span>
        </template>
      </el-table-column>
      <el-table-column label="适用类型" align="center" v-if="columns.type.visible">
        <template slot-scope="scope">
          <span>{{ getProcessTypeLabel(scope.row.typeId || scope.row.type) }}</span>
        </template>
      </el-table-column>
      <el-table-column label="描述" align="center" v-if="columns.description.visible" :show-overflow-tooltip="true">
        <template slot-scope="scope">
          <span>{{ scope.row.progressDesc || scope.row.description }}</span>
        </template>
      </el-table-column>
      <el-table-column label="状态" align="center" v-if="columns.active.visible">
        <template slot-scope="scope">
          <el-switch
            :value="scope.row.enabled === 1 || scope.row.active === true"
            @change="toggleProcess(scope.row.id, $event)"
            active-text="启用"
            inactive-text="停用"
          />
        </template>
          ...
          <div class="wflow-basic-setting">
            ...
            <div class="setting-section">
              ...
              <div class="group-setting">
                <el-select v-model="form.type" placeholder="请选择分组" style="width: 300px">
                  <el-option v-for="t in processTypeList" :key="t.id" :label="t.name" :value="String(t.id)" />
                </el-select>
  ...
  methods: {
    ...
    handleUpdate(row) {
      ...
      getManage(id).then(response => {
        ...
        this.form.name = data.progressName || data.name || ''
        this.form.type = data.typeId ? String(data.typeId) : (data.type || '')
        this.form.description = data.progressDesc || data.description || ''
        this.form.active = data.enabled === 1 || data.active === true
        this.form.remark = data.remark || ''
        // 解析 progressContent JSON
        try {
          const content = JSON.parse(data.progressContent || '{}')
        ...
        // 通知方式
        if (data.notifyType) {
          this.form.settings.notify.type = data.notifyType
        }
        if (data.notifyTitle) {
          this.form.settings.notify.title = data.notifyTitle
        }
        this.open = true
        this.title = "修改流程管理"
      })
    ...
    buildSubmitData() {
      ...
      return {
        id: this.form.id,
        progressName: this.form.name,
        typeId: this.form.type ? parseInt(this.form.type) : null,
        progressDesc: this.form.description,
    ...
    submitForm() {
      ...
      if (!submitData.progressName) {
        this.$message.warning("请输入流程名称")
        return
      }
      if (submitData.id != null) {
        updateManage(submitData).then(response => {
          this.$message.success("修改成功")
          this.open = false
          this.getList()
        })
      } else {
        addManage(submitData).then(response => {
    ...
    getProcessTypeLabel(typeId) {
      // 先从动态加载的类型列表中查找
      const found = this.processTypeList.find(t => t.id === typeId || String(t.id) === String(typeId))
      if (found) return found.name
      // 兼容旧数据
      const typeMap = {
        '1': '财务', '2': '人事', '3': '行政', '4': '其他',
        'expense': '费用报销', 'leave': '请假申请',
        'purchase': '采购申请', 'business': '出差申请'
      }
      return typeMap[String(typeId)] || typeId || '未分类'
    },

    /** 加载流程类型列表 */
    loadProcessTypes() {
      // 从 sys_progress_type 表加载
      this.getDicts("sys_progress_type").then(response => {
        if (response && response.data) {
          this.processTypeList = response.data
        }
      }).catch(() => {
        // 如果字典接口不可用，尝试直接查询
        listManage({ pageNum: 1, pageSize: 1 }).then(() => {
          // 使用默认类型
