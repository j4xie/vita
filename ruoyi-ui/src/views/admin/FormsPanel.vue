<template>
  <div class="from-panel" ref="panel">
    <div class="from-title">
      <el-button icon="el-icon-back" type="info" size="mini" circle plain style="margin-right: 15px" @click="$router.go(-1)"></el-button>
      <span>流程面板</span>
...
export default {
  ...
  methods: {
    ...
    newProcess(groupId) {
      ...
      this.$emit('openDesigner', { groupId: groupId, formId: null });
    },
    groupSort() {
    ...
    editFrom(item, group) {
      this.$emit('openDesigner', { groupId: group ? group.id : null, formId: item.formId });
    },
    stopFrom(item) {
