...
public class SysUserServiceImpl implements ISysUserService
{
    ...
    public List<SysUser> selectUserList(SysUser user)
    {
        ...
        for(int i = 0 ;i < list.size(); i++){
            if(null != list.get(i).getDept()){
                if(null != list.get(i).getDept().getParentId() && list.get(i).getDept().getParentId() > 1){
                    //获取父级部门并拼上
                    SysDept sysDept = deptService.selectDeptById(list.get(i).getDept().getParentId());
                    if(null != sysDept){
    ...
    public List<SysUser> selectUserListForApp(SysUser sysUser){
        List<SysUser> list = userMapper.selectUserListForApp(sysUser);
        for(int i = 0 ;i < list.size(); i++){
            if(null != list.get(i).getDept()){
                if(null != list.get(i).getDept().getParentId() && list.get(i).getDept().getParentId() > 1){
                    //获取父级部门并拼上
    ...
    {
        return userMapper.selectUserLevelAndPoints(userExtendsData);
    }
    ...
    public int updateUserVerifyEmail(SysUser user){
        return userMapper.updateUserVerifyEmail(user.getUserId(), user.getIsEmailVerify());
    }
    ...
    public int resetUserPwdByPhoneNumber(String phonenumber, String password){
        return userMapper.resetUserPwdByPhoneNumber(phonenumber, password);
    }
